package org.springframework.data.influxdb.mapper;
...
/**
 * Created by @author yihui in 16:53 19/5/5.
 */
public class BigDecimalInfluxDBResultMapper {
    ...
    private static final DateTimeFormatter RFC3339_FORMATTER =
            new DateTimeFormatterBuilder().appendPattern("yyyy-MM-dd'T'HH:mm:ss")
                    .appendFraction(ChronoField.NANO_OF_SECOND, FRACTION_MIN_WIDTH, FRACTION_MAX_WIDTH,
                            ADD_DECIMAL_POINT).appendZoneOrOffsetId().toFormatter();
    ...
    void cacheMeasurementClass(final Class<?>... classVarAgrs) {
        for (Class<?> clazz : classVarAgrs) {
            ...
            ConcurrentMap<String, Field> influxColumnAndFieldMap =
    ...
    <T> boolean fieldValueModified(final Class<?> fieldType, final Field field, final T object, final Object value,
            final TimeUnit precision) throws IllegalArgumentException, IllegalAccessException {
